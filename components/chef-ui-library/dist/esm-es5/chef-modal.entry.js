 ...
 * </chef-modal>
 */
var ChefModal = /** @class */ (function () {
    function ChefModal(hostRef) {
    ...
    ChefModal.prototype.handleEscape = function (event) {
        if (event.key === 'Escape') {
            this.handleClose();
        }
    };
    ChefModal.prototype.setFocus = function (visible) {
        if (visible) {
            this.prevFocusedElement = document.activeElement;
            var focusElement_1 = this.getFocusElement(this.locked);
            var focusElementInterval_1 = setInterval(function () {
                focusElement_1.focus();
                if (focusElement_1 === document.activeElement) {
                    clearInterval(focusElementInterval_1);
                }
            }, 1);
        }
    };
    ChefModal.prototype.render = function () {
        return (h(Host, { class: this.visible ? 'visible' : '' }, h("div", { class: "modal-overlay", onClick: this.handleClose.bind(this) }), h("div", { class: "modal", "aria-modal": "true", role: "dialog", "aria-labelledby": this.label, tabindex: "0" }, h("chef-trap-focus", null, this.renderButton(), h("slot", { name: "title" }), h("slot", null)))));
    };
    ChefModal.prototype.renderButton = function () {
        if (!this.locked) {
            return (h("chef-button", { class: "close", onClick: this.handleClose.bind(this), secondary: true }, h("chef-icon", null, "close")));
        }
        return '';
    };
    ...
    ChefModal.prototype.getFocusElement = function (lockStatus) {
        var modal = this.el.getElementsByClassName('modal').item(0);
        var closeFocus = this.el.getElementsByClassName('close').item(0).firstElementChild;
        var firstFocus = this.el.querySelector('[firstFocus]');
        if (lockStatus) {
            return modal;
        }
        else if (firstFocus) {
    ...
    ChefModal.prototype.handleClose = function () {
        var _this = this;
        if (!this.locked) {
    ...
    Object.defineProperty(ChefModal.prototype, "el", {
        get: function () { return getElement(this); },
        enumerable: true,
        configurable: true
    });
    Object.defineProperty(ChefModal, "watchers", {
        get: function () {
            return {
                "visible": ["setFocus"]
            };
    ...
    Object.defineProperty(ChefModal, "style", {
        get: function () { return "chef-modal{display:none}chef-modal.visible{display:block}chef-modal .modal-overlay{position:fixed;top:0;left:0;z-index:1000;width:100%;height:100%;background-color:var(--chef-modal-bg-color)}chef-modal .modal{position:fixed;top:50%;left:50%;-webkit-transform:translateX(-50%) translateY(-50%);transform:translateX(-50%) translateY(-50%);z-index:1001;background-color:var(--chef-white);color:var(--chef-primary-dark);border-radius:var(--chef-border-radius);padding:30px;--default-modal-width:700px;width:var(--modal-width,var(--default-modal-width))}chef-modal .modal .close{position:absolute;top:5px;right:5px;width:auto}chef-modal .modal [slot=title]{--default-title-size:24px;font-size:var(--title-size,var(--default-title-size))}"; },
        enumerable: true,
        configurable: true
    });
    return ChefModal;
}());
export { ChefModal as chef_modal };
